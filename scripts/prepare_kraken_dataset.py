"""Prepare manifest CSVs as a Kraken legacy line-strip dataset."""
...
def main() -> None:
    ...
    if output_dir is None:
        output_dir = KRAKEN_DATA_DIR / f"fold{args.fold}_h{args.image_height}"
    ...
    print(f"Prepared Kraken dataset: {paths.root}")
    print(f"Train dir: {paths.train_dir}")
    print(f"Validation dir: {paths.val_dir}")
    print(f"Test dir: {paths.test_dir}")
    print(f"Train files: {paths.train_files}")
    print(f"Validation files: {paths.val_files}")
    print(f"Test files: {paths.test_files}")
